...
A natural 3-class split (Low $\leq 4$, Medium 5--6, High $\geq 7$) produces severe imbalance: 90.8\% High. We use \textbf{tertile-based boundaries} ($\tau_1 = 6.80$, $\tau_2 = 7.40$), producing near-balanced classes: Low 34.8\%, Medium 37.3\%, High 27.9\%. The final dataset has \textbf{1,965 samples} with 78 features (65 standard + 13 creative).
...
For each patient, the first 80\% of days (chronologically) form the training set and the last 20\% the test set --- a \textbf{temporal split} that prevents data leakage. A random split would allow the model to exploit mood's strong autocorrelation, inflating performance estimates. The split yields 1,559 training and 406 test samples. We report \textbf{weighted F1} as the primary metric, as it combines precision and recall weighted by class support and handles the remaining class imbalance better than accuracy.

\subsection{Model 1: Random Forest}

Random Forest~\cite{breiman2001} is an ensemble of decision trees that reduces overfitting through bagging and random feature subsets. It handles mixed tabular features and gives interpretable importance rankings. Input: 78 features standardized with StandardScaler (fit on training data only).

\textbf{Hyperparameter optimization:} RandomizedSearchCV with TimeSeriesSplit (5-fold inner CV, 30 iterations). Best configuration: n\_estimators $= 100$, max\_depth $= 5$, min\_samples\_split $= 2$, class\_weight $=$ \texttt{balanced\_subsample}. Inner CV weighted F1 $= 0.710$.

Test performance: \textbf{accuracy 0.562}, \textbf{weighted F1 0.555}. Per-class results in Table~\ref{tab:cls_results}.

\subsection{Model 2: LSTM}
...
Gradient Boosting~\cite{friedman2001} was also tested as an additional baseline (accuracy 0.539, F1 $= 0.535$).

\begin{table}[H]
\centering
\caption{Classification results on the held-out test set (406 samples, tertile-based classes).}\label{tab:cls_results}
...
\midrule
Low    & 0.55 & 0.71 & 0.62 & 0.49 & 0.63 & 0.55 \\
Medium & 0.56 & 0.42 & 0.48 & 0.49 & 0.25 & 0.33 \\
High   & 0.58 & 0.58 & 0.58 & 0.44 & 0.59 & 0.50 \\
\midrule
\textbf{Weighted avg} & \textbf{0.56} & \textbf{0.56} & \textbf{0.55} & \textbf{0.48} & \textbf{0.47} & \textbf{0.45} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{(a) RF confusion matrix: Low mood is most reliably detected (recall 0.71). (b) Top feature importances: mood\_last, mood\_min, and the creative feature \textbf{mood\_ema} rank 1--3, followed by mood\_mean.}\label{fig:rf}
\end{figure}

\subsection{Discussion}

Random Forest substantially outperforms LSTM (F1: 0.555 vs.\ 0.452). We identify four reasons:

\textbf{1. Engineered features provide explicit predictive signal.} Feature importances (Fig.~\ref{fig:rf}b) show mood\_last (12.0\%), mood\_min (11.4\%), mood\_ema (11.2\%), and mood\_mean (10.5\%) as the top four features, accounting for $>$45\% of total importance. The EMA feature, which we designed to capture mood momentum, ranks 3rd overall, validating the creative feature design. The circumplex quadrant feature ranks 8th, confirming that the arousal$\times$valence interaction adds information beyond the raw dimensions separately.
